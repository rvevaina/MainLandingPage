...
function conversions_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'conversions' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'conversions' ),
...
function conversions_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && conversions_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'conversions' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'conversions' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'conversions' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
...
function conversions_categorized_blog() {
	...
	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so conversions_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so conversions_categorized_blog should return false.
		return false;
	}
}
